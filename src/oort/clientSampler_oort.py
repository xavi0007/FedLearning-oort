from src.clientsFile.client import Client 
import math
...
class clientSampler_Oort(object):

    def __init__(self, mode, score, batch_size, local_epochs, filter=0, sample_seed=233 ):
        self.Clients = {}
        self.clientOnHosts = {}
        self.mode = mode
        self.score = score
        self.filter_less = 32
        self.filter_more = 1e5

        self.ucbSampler = create_training_selector() if self.mode == 'oort' else None
        ...
        self.upload_epoch = local_epochs

        if self.user_trace is not None:
            with open(self.user_trace, 'rb') as fin:    
    ...
    def registerClient(self, hostId, clientId, dis, size, speed=[1.0, 1.0], duration=1):
        ...
        if size >= self.filter_less and size <= self.filter_more:
            ...
            if self.mode == "oort":
                feedbacks = {'reward':min(size, self.upload_epoch*self.batch_size),
                            'duration':duration,
                            }
