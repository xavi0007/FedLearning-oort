...
class _training_selector(object):
    ...
    def getTopK(self, numOfSamples, cur_time, feasible_clients):
        ...
        if self.round_threshold < 100.:
            all_durations = [self.totalArms[key]['duration'] for key in client_list]
            self.round_prefer_duration = min(all_durations)
        else:
            self.round_prefer_duration = float('inf')
        ...
        for key in orderedKeys:
            ...
            if self.totalArms[key]['count'] > 0:
                ...
                if self.totalArms[key]['time_stamp'] == 0:
                    self.totalArms[key]['time_stamp'] = 1
                #exploitation 1: calculate utility
        ...
        #Exploitation 2: Admit clients with greater than c%(5%) of cut-off utility Sample only (1e-4)K clients by utility 
        sortedClientUtil = sorted(scores, key=scores.get, reverse=True)

        # take cut-off utility
        cut_off_util = scores[sortedClientUtil[exploitLen-1]] * self.cut_off_util
