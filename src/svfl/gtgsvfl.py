...
class GTGShapleyValue(ShapleyValue):
    def __init__(self, worker_number: int, last_round_metric: float = 0):
        ...
        print(f"max_number {self.max_number}")
        logging.info("max_number %s", self.max_number)
    ...
    def compute(self):
        ...
        logging.info("shapley_value %s", self.shapley_values[self.round_number])
        logging.info(
    ...
    def not_convergent(self, index, contribution_records):
        if index >= self.max_number:
            logging.info("convergent for max_number %s", self.max_number)
            return False
